...
// @version      29.0
// @description  Dashboard + Auto Auth (Modal) + One Click Sync (No Alert) + State Persistence + Viewer Shortcuts
// @author       System Admin
...
(function() {
    ...
    // --- 1. KONFIGURASI URL & AUTH ---
    const BASE_API_URL = "https://owo-zyrex.mars-project.my.id";
    const AUTH_API_URL = "https://owo-zyrex.mars-project.my.id/login";
    const TOKEN_EXPIRY_MS = 3 * 60 * 60 * 1000; // 3 Jam

    // --- VARIABLE STATE (AGAR DATA TIDAK HILANG SAAT PINDAH FOTO) ---
    let formState = {
        dropdowns: {},
        sn_manual: '',
        tgl_manual: ''
    ...
    const REASON_MAPPING = {
        ...
        "bapp_hal1_Data NPSN sekolah tidak sesuai": "(1K) Data BAPP sekolah tidak sesuai",
        ...
        "bapp_hal1_Pihak pertama hanya boleh dari kepala sekolah/wakil kepala sekolah/guru/pengajar/operator sekolah": "(1AN) Pihak pertama hanya boleh dari kepala sekolah/wakil kepala sekolah/guru/pengajar/operator sekolah",
    ...
    document.getElementById('sys-btn-login-action').addEventListener('click', function() {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                ...
                if (response.status === 200) {
                    ...
                    alert("Login Gagal! Cek username/password. (" + response.status + ")");
                }
            },
    ...
    const pageData = {
        ...
        alamat: (function(){
            ...
            let addr = [];
            inputs.forEach(input => { if(input.value) addr.push(input.value); });
            return addr.length > 0 ? addr.join(', ') : '-';
        })(),
        noBapp: document.getElementById('no_bapp')?.value || '-'
    };

    // --- 7. INJECT TOMBOL (ONE CLICK SYNC) ---
    function injectActionButtons() {
        const buttonContainer = document.querySelector('.login-horizental');

        if (buttonContainer) {
            const originalSubmit = buttonContainer.querySelector('button[type="submit"], .btn-login');
            if (originalSubmit) {
                originalSubmit.style.display = 'none'; // Hide Original Button
            }
            ...
            if (!document.getElementById('sys-btn-approve-main')) {
                ...
                approveBtn.className = 'btn btn-primary waves-effect waves-light btn-custom-approve';
                ...
                approveBtn.onclick = function(e) {
                    ...
                    if (autoReason) {
                        alert("PERINGATAN: Form Evaluasi masih ada item bermasalah:\n\n" + autoReason + "\n\nPerbaiki jadi 'Sesuai' jika ingin Menerima.");
                        return;
                    }
                    if (confirm(`Yakin SIMPAN data ke Asshal dan TERIMA di Zyrex?\n\nNPSN: ${pageData.npsn}`)) {
                ...
                rejectBtn.className = 'btn btn-danger waves-effect waves-light btn-custom-reject';
    ...
    // --- 8. API CALL & AUTO SUBMIT (NO ALERT) ---
    function callApi(btnElement, actionType, reason) {
        const originalText = btnElement.innerHTML;
        const url = `${BASE_API_URL}/${actionType}`;

        btnElement.disabled = true;
        btnElement.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Memproses Zyrex...';
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                ...
                if (response.status >= 200 && response.status < 300) {
                    btnElement.innerHTML = '<i class="fa fa-check"></i> Menyimpan Asshal...';
                    ...
                    btnElement.innerHTML = originalText;
                    let errMsg = "Unknown";
                    try { errMsg = JSON.parse(response.responseText).error || response.responseText; } catch(e) {}
                    alert(`❌ GAGAL ZYREX (${actionType.toUpperCase()})!\nData Asshal BELUM Disimpan.\n\nServer: ${response.status}\nPesan: ${errMsg}`);
                }
            },
            onerror: function(err) {
                btnElement.disabled = false;
    ...
    function generateAutoReason() {
        ...
        for (const [key, val] of Object.entries(formState.dropdowns)) {
            if (!val || val === "Sesuai" || val === "Ada" || val === "Lengkap" || val === "Jelas" || val === "Konsisten") continue;

            const safeKeywords = ['sesuai', 'ada', 'lengkap', 'jelas', 'terbaca', 'konsisten'];
            const valLower = val.toLowerCase();
            const isSafe = safeKeywords.some(keyword => valLower.includes(keyword) && !valLower.includes('tidak') && !valLower.includes('belum'));
            ...
            if (!isSafe) {
                ...
                if (!mappedReason) {
                    ...
                    const foundKey = Object.keys(REASON_MAPPING).find(k => k.toLowerCase() === searchKeyLower);
                    if (foundKey) mappedReason = REASON_MAPPING[foundKey];
                }
                if (mappedReason) {
                    issues.push(mappedReason);
                } else {
                    const originalSelect = document.querySelector(`select[name="${key}"]`);
                    let label = key;
                    if (originalSelect) {
                        const row = originalSelect.closest('.form-group') || originalSelect.closest('.sys-form-row');
                        if(row) {
                            const lbl = row.querySelector('label') || row.querySelector('.login2');
    ...
    // --- 10. Request Tracking (AWB) ---
    if (pageData.npsn !== '-' && pageData.npsn) {
        const loadDiv = document.createElement('div');
        loadDiv.style.cssText = "position:fixed;bottom:10px;right:10px;background:#2c3e50;color:#fff;padding:10px 20px;border-radius:30px;z-index:9999999;font-size:12px;font-weight:bold;";
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                loadDiv.remove();
                if (response.status === 200) {
                    try { initSystem(JSON.parse(response.responseText)); } catch (e) {}
                } else if (response.status === 401 || response.status === 403) {
                    localStorage.removeItem('access_token_v1');
                    showLoginModal();
                }
            },
            onerror: function() { loadDiv.remove(); }
        });
    }

    // --- 11. Inisialisasi Sistem ---
    ...
    function renderDashboard(data) {
        ...
        if (photoKeys.length > 0) {
            photoKeys.forEach(key => { html += `<div class="sys-gallery-item"><img src="${photos[key]}" alt="${key}"><div class="sys-gallery-caption">${key}</div></div>`; });
        } else { html += `<p style="padding:10px;text-align:center;color:#999;">Tidak ada foto.</p>`; }
        ...
        history.forEach(h => { html += `<tr><td style="white-space:nowrap;">${h.date}</td><td>${h.location_code || '-'}</td><td><strong>${h.status_code}</strong></td><td>${h.status}<br><small style="color:#888;">${h.status_desc}</small></td></tr>`; });
        ...
        if (galleryEl && photoKeys.length > 0) {
            viewerInstance = new Viewer(galleryEl, {
                title: false, navbar: true, zoomRatio: 0.2, transition: false, tooltip: true, movable: true, toolbar: { zoomIn: 1, zoomOut: 1, rotateLeft: 1, rotateRight: 1, prev: 1, next: 1, reset: 1 },
                view: function() { setTimeout(() => { injectStickyBoxesToViewer(); }, 100); }
            });
        }
    }
    ...
    document.addEventListener('keydown', function(e) {
        ...
        // PENGECUALIAN: Hanya blokir jika di INPUT TEXT/PASSWORD atau TEXTAREA
        // Agar shortcut tetap jalan di Dropdown (SELECT) dan Date
        if (activeEl && (
            (activeEl.tagName === 'INPUT' && (activeEl.type === 'text' || activeEl.type === 'password')) ||
        ...
        if (viewerContainer && viewerInstance) {
            ...
            if (allowedKeys.includes(key)) {
                // PAKSA BERHENTI EVENT (Agar dropdown tidak bereaksi)
                e.preventDefault();
                e.stopPropagation();
    ...
    // --- 12. Sticky Boxes (Inject Mode with STATE) ---
    function injectStickyBoxesToViewer() {
        ...
        ['mousedown', 'mouseup', 'click', 'touchstart', 'touchend', 'keydown', 'keyup'].forEach(evt => {
            rightBox.addEventListener(evt, function(e) { e.stopPropagation(); });
        });
    ...
    function setupSyncLogic(boxElement) {
        ...
        stickySelects.forEach(select => {
            ...
            select.addEventListener('change', function() {
                ...
                formState.dropdowns[tName] = val; // SAVE STATE
        ...
        if (snBox && snReal) {
            ...
            snBox.addEventListener('input', function() {
                formState.sn_manual = this.value.toUpperCase(); // SAVE STATE
        ...
        if (dateBox && dateReal) {
            ...
            dateBox.addEventListener('wheel', function(e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    function setBestDefault(selectElement) {
        ...
        for (let i = 0; i < selectElement.options.length; i++) {
            ...
            if (bestKeywords.some(keyword => optText.includes(keyword))) {
                ...
                syncDropdown(selectElement.getAttribute('data-target'), optVal, pageJQuery);
                if(selectElement.getAttribute('data-target') === 'ket_tgl_bapp') handleDateVisibility(optVal);
                if(selectElement.getAttribute('data-target') === 'bc_bapp_sn') handleSNLogic(optVal);
                return;
            }
        }
    }
    ...
    function handleDateVisibility(value) {
        ...
        if (value === 'Sesuai') {
            ...
            // Cek jika input kosong dan belum ada data manual
            if (dateInput && !dateInput.value && !formState.tgl_manual) {

                // GANTI: Jangan pakai 'new Date()' (hari ini)
                // TAPI pakai tanggal statis: 25 Desember 2025
                const targetDate = new Date('2025-12-25');

                const year = targetDate.getFullYear();
                const month = String(targetDate.getMonth() + 1).padStart(2, '0');
                const day = String(targetDate.getDate()).padStart(2, '0');
                const isoDate = `${year}-${month}-${day}`; // Hasil: 2025-12-25

                dateInput.value = isoDate;
                formState.tgl_manual = isoDate; // Simpan ke state

                // Update Datepicker Asli Web Asshal
                if(realDateInput && pageJQuery && typeof pageJQuery(realDateInput).datepicker === 'function') {
                    pageJQuery(realDateInput).datepicker('update', targetDate);
                }
            }
            // -----------------------------
    ...
    function handleSNLogic(value) {
        ...
        if (value === 'Ada') {
            if (pageData.snPenyedia !== '-' && !formState.sn_manual) {
                ...
                formState.sn_manual = pageData.snPenyedia; // Save state
            }
        }
    }
